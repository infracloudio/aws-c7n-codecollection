...
Metadata            Display Name    AWS Compliance
Documentation        Counts the number of AWS resources that do not follow compliance.
...
*** Tasks ***
Check for Missing Tags on AWS ${AWS_RESOURCE_PROVIDERS} resources in AWS Region `${AWS_REGION}` in AWS account `${AWS_ACCOUNT_ID}` 
    ...
    ...    cmd=custodian run -r ${AWS_REGION} --output-dir ${OUTPUT_DIR}/aws-c7n-compliance ${CURDIR}/tag-compliance.yaml --cache-period 0
    ...    secret__aws_access_key_id=${AWS_ACCESS_KEY_ID}
    ...    secret__aws_secret_access_key=${AWS_SECRET_ACCESS_KEY}
    ${dirs}=    RW.CLI.Run Cli
    ...    cmd=find ${OUTPUT_DIR}/aws-c7n-compliance -mindepth 1 -maxdepth 1 -type d | jq -R -s 'split("\n") | map(select(length > 0))';
...
Suite Initialization
    ...
    ...    type=string
    ...    description=The minimum number of AWS resources missing tags.
    ...    pattern=^\d+$
    ...    example=2
    ...    default=1
    ...
    ${clean_workding_dir}=    RW.CLI.Run Cli    cmd=rm -rf ${OUTPUT_DIR}/aws-c7n-compliance         # Note: Clean out the cloud custoding report dir to ensure accurate data
