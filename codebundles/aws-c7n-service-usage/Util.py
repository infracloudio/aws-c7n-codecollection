...
def parse_custodian_log(log_file_path,usage_percentage):
    data = []
    pattern = re.compile(r'custodian\.filters:INFO\s+(?P<service>.*?)\s+usage:\s+(?P<used>\d+\.?\d*)/(?P<limit>\d+\.?\d*)')
    ...
    try:
        ...
        return f"Error: Permission denied when trying to read '{log_file_path}'."
    ...
    for line in log_lines:
        ...
        if match:
            ...
            print(usage_pct)
            if usage_pct >= float(usage_percentage):
                data.append((service, used, limit, f'{usage_pct:.2f}%'))
    
    headers = ["Service Description", "Used", "Limit", "Usage %"]
    return {"data": data, "report_table": tabulate(data, headers=headers, tablefmt="github")}
...
# print(parse_custodian_log('./service-usage.log', threshold=0))
